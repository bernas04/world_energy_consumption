...
const firstYearData = 1900;


var name_iso = {}

for (var iso in jvmCountries) {
    name_iso[jvmCountries[iso].name] = iso
}
...
readTextFile("../../data.json", function(text, map){
    var data = JSON.parse(text);
        
    // Buscar os isoCodes e dados
    var iso_values = {};
    
    for (var i = 0; i < data.length; i++) {
        var country = Object.keys(data[i])[0]; //get the country name

        if (country in name_iso) {
            var iso = name_iso[country];
            var value = parseInt(data[i][country][0].population);
            if (value!=null) {
                iso_values[iso] = value;
            }
        }
    }
...
//map.params.visualizeData.values = iso_values;
